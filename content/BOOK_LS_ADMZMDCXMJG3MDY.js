...
const BOOK_LS_ADMZMDCXMJG3MDY = () => {
    ROUTER_LS_YTE4MTK0NDQZMTY.get("/:title-book-:id", (req, res) => {
        try {
            ...
            LS_YTIZNTY3ODE4NJC({active: book_id}).ELEMENT().then(book_info => {
                hot_category = THIS.METHOD.EXCEPTIONS.SET_START_CONTENT({ content: book_info })
                posive()
            }).catch(error => {
            ...
            const get_best_content = ({info}) => {
                LS_DDE4NJKZMTA1NZE({ current: current, bookId: book_id, name: info.name , href: req.originalUrl}).ELEMENT().then(content => {
            ...
            LS_CTIWOTU4MTYZNDM().GET_BOOK({objectId: MONGODB().METHOD.ID_TO_OBJECT({id: book_id})}).then(book_info => {
                get_best_content({info: book_info})
            })
...
module.exports.BOOK_LS_ADMZMDCXMJG3MDY = BOOK_LS_ADMZMDCXMJG3MDY